...
define retty label argtys body =
        addDefn $
                GlobalDefinition $
                        functionDefaults
                                { name = Name $ B.Short.toShort $ C.pack label
...
emptyCodegen = CodegenState (Name $ B.Short.toShort $ C.pack entryBlockName) M.empty [] 1 0 M.empty
...
addBlock bname = do
        ...
        modify $ \s ->
                s
                        { blocks = M.insert (Name $ B.Short.toShort $ C.pack qname) new bls
                        , blockCount = ix + 1
                        , names = supply
                        }
        return (Name $ B.Short.toShort $ C.pack qname)
...
uncons :: Operand -> C.Constant
uncons (ConstantOperand a) = a
uncons _ = error "not constant"
